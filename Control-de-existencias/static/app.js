...
async function cargarDashboard() {
    try {
        // Cargar alertas
        const alertas = await llamarAPI('GET', '/v1/alertas');
        ...
        // Cargar reporte
        const reporte = await llamarAPI('GET', '/v1/reporte');
...
async function cargarProductos() {
    try {
        const productos = await llamarAPI('GET', '/v1/productos');
...
document.getElementById('form-producto').addEventListener('submit', async (e) => {
    ...
    try {
        await llamarAPI('POST', '/v1/productos', datos);
...
async function eliminarProducto(id) {
    ...
    try {
        await llamarAPI('DELETE', `/v1/productos/${id}`);
...
document.getElementById('form-entrada').addEventListener('submit', async (e) => {
    ...
    try {
        await llamarAPI('POST', '/v1/entrada', datos);
...
document.getElementById('form-salida').addEventListener('submit', async (e) => {
    ...
    try {
        await llamarAPI('POST', '/v1/salida', datos);
...
document.getElementById('form-ajuste').addEventListener('submit', async (e) => {
    ...
    try {
        await llamarAPI('POST', '/v1/ajuste', datos);
